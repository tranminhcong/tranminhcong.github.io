...
function ListChapProcess(b, c, d, e, f) {
    ...
    window.history.pushState({
        ...
        truyen_ascii: c,
        page: d
    }, "Truy\u1ec7n " + b + " - Trang " + d, "/" + c + "/trang-" + d + "/#list-chapter");
    document.title = "Truy\u1ec7n " + b + " - Trang " + d;
    ga("send", "pageview", {
        page: "/" + c + "/trang-" + d + "/",
        title: "Truy\u1ec7n " + b + " - Trang " + d + " - Ajax"
    })
}
function FormError(b, c) {
    c ? $("#" + c + " .single-page-status").removeClass("alert-success hide").addClass("alert-danger").html(b) : ($(".single-page-status").removeClass("alert-success hide").addClass("alert-danger").html(b),
    $("html, body").animate({
        scrollTop: $(".single-page-status").offset().top
    }, "slow"))
}
function FormSuccess(b, c) {
    c ? $("#" + c + " .single-page-status").removeClass("alert-danger hide").addClass("alert-success").html(b) : ($(".single-page-status").removeClass("alert-danger hide").addClass("alert-success").html(b),
    $("html, body").animate({
        scrollTop: $(".single-page-status").offset().top
    }, "slow"))
}
...
function UsernameValidate(b) {
    if (b.match(/^[a-z][a-z0-9]*[_\.]?[a-z0-9]+$/) && (b = b.length,
    3 <= b && 18 >= b))
        return "ok"
}
...
function load_comment(b, c, d) {
    ...
    if ("undefined" !== typeof FB)
        FB.XFBML.parse(document.getElementById(b));
    else
        var e = setInterval(function() {
            "undefined" !== typeof FB && (FB.XFBML.parse(document.getElementById(b)),
            clearInterval(e))
        }, 100)
}
...
function nextNode_remove(b) {
    var c = $("div#" + b)[0].nextSibling;
    c && 3 != c.nodeType && $("div#" + b).next("br").remove();
...
$(document).ready(function() {
    ...
    if (g) {
        "/" != window.location.pathname && function(a, b, c) {
            ...
            a.id = c,
            a.src = "https://connect.facebook.net/vi_VN/all.js#version=v3.1&appId=333184773543272",
    ...
    if (h) {
        ...
        1 == C && ($("#fluid-no").attr("checked", !1),
        $("#fluid-yes").attr("checked", !0));
        1 == D ? ($("#onebreak-no").attr("checked", !1),
        ...
        $(document).keydown(function(a) {
            !1 !== $("input[type='text'], input[type='search']").is(":focus") || a.ctrlKey || (37 == a.which || 65 == a.which ? $("#prev_chap")[0].click() : 39 == a.which || 68 == a.which ? $("#next_chap")[0].click() : 83 == a.which ? (a = $(window).scrollTop(),
            $(window).scrollTop(a + 37)) : 87 == a.which && (a = $(window).scrollTop(),
            $(window).scrollTop(a - 37)))
        });
        ...
        $(".chapter-nav").on("click", "button#chapter_error", function() {
            ...
            if (null === a || !1 === a)
                return !1;
            "" === a ? alert("B\u1ea1n ch\u01b0a nh\u1eadp m\u00f4 t\u1ea3. B\u00e1o l\u1ed7i kh\u00f4ng th\u00e0nh c\u00f4ng.") : ($("button#chapter_error").addClass("hide"),
            $.post("/ajax.php", {
                type: "chapter_error",
                id: $("#chapter-id").val(),
                message: a
    ...
    $("#search-input").keyup(function() {
        ...
        clearTimeout(p);
        a = $(this).val();
        0 == a.length ? $(".list-search-res").html("").addClass("hide") : 3 <= a.length && ($(".list-search-res").html('<img src="img/loading-search.gif" alt="loading">').removeClass("hide"),
    ...
    $("#search-input").bind("paste", function() {
        ...
        setTimeout(function() {
            var b = $(a).val();
            0 == b.length ? $(".list-search-res").html("").addClass("hide") : 3 <= b.length && ($(".list-search-res").html('<img src="img/loading-search.gif" alt="loading">').removeClass("hide"),
            p = setTimeout(function() {
                quick_search(b)
            }, 500))
        }, 0)
    ...
    if ($("#body_home").length) {
        ...
        $.get("/ajax.php", {
            ...
            a && ($("#history-holder").replaceWith(a.main),
            $("#history-holder-side").replaceWith(a.side))
        });
    ...
    $(".rate-holder").raty({
        ...
        readOnly: function() {
            if ("rated" == localStorage.getItem(g))
                return !0
        }
    });
    randomso(1, 8);
